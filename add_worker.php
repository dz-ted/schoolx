...
$user_sql = $conn->prepare("SELECT * FROM users WHERE username=? LIMIT 1");
$user_sql->bind_param("s", $user_id);
$user_sql->execute();
$user_data = $user_sql->get_result()->fetch_assoc();
$user_sql->close();

// التحقق من صلاحية إضافة عامل
if(!$user_data || $user_data['can_add'] != 1){
    exit("❌ ليست لديك صلاحية إضافة عامل!");
}

// المرحلة المسموح بها للمستخدم
$user_stage = $user_data['stage'] ?? 'ابتدائي';
...
if(isset($_POST['submit'])){
    ...
    // ✅ رخصة السياقة نصية (يوجد / لا يوجد)
    $driving_license = $_POST['driving_license'] ?? 'لا يوجد';
$driving_license = ($driving_license === 'يوجد') ? 'يوجد' : 'لا يوجد';
    ...
    try {
        $stmt = $conn->prepare("INSERT INTO workers_data 
        (employee_id, full_name, birth_date, nationality, marital_status, num_children, driving_license, job_title, experience, degree, major, university, graduation_year, courses, skills, year_of_assignment, phone_mobile, phone_home, emergency_phone, address_qatar, address_home, employee_number, sponsorship, photo, stage, role)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

      $stmt->bind_param(
    "ssssssssisssssssisssssssss",
    $employee_id, $full_name, $birth_date, $nationality, $marital_status,
    $num_children, $driving_license, $job_title, $experience, $degree,
    $major, $university, $graduation_year, $courses, $skills,
    $year_of_assignment, $phone_mobile, $phone_home, $emergency_phone,
    $address_qatar, $address_home, $employee_number, $sponsorship,
    $photo, $stage, $role
);
        $stmt->execute();
        $conn->commit();
...
<select name="stage" required>
    <option value="<?= htmlspecialchars($_SESSION['stage']) ?>" selected>
        <?= htmlspecialchars($_SESSION['stage']) ?>
    </option>
</select>
...
<label>تاريخ الميلاد</label><input type="date" name="birth_date">
...
<option value="عامل وسائق">عامل وسائق</option>
